...
%nterm<std::string> command_invocation
...
file_element
    : space_list command_invocation space_list file_line_ending {
        ctx.function.Name = std::move($2);
        ctx.function.Line = @2.begin.line;
...
command_invocation
    : IDENTIFIER {
        ctx.function.rpnExpr.Push<StringExpression>($1);
...
reference
    ...
    | REF_OPEN var_reference_list REF_CLOSE {
        ...
        case ReferenceType::Env:
            ctx.function.rpnExpr.Push<EnvVarRefExpression>($$);
            break;
        }
    }
